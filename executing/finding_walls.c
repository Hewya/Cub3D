/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   finding_walls.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gabarnou <gabarnou@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/10 11:26:28 by gabarnou          #+#    #+#             */
/*   Updated: 2024/07/15 01:36:27 by gabarnou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	find_first_hor(t_data *data, double angle, t_pos *hor_int,
		t_wall *hor_wall)
{
	if (angle < M_PI)
	{
		hor_int->y = (int)(data->player_pos.y / BLOCK_RES) * BLOCK_RES;
		hor_wall->walldirection = 'S';
	}
	else
	{
		hor_int->y = (int)(data->player_pos.y / BLOCK_RES) * BLOCK_RES
			+ BLOCK_RES;
		hor_wall->walldirection = 'N';
	}
	hor_int->x = data->player_pos.x + ((data->player_pos.y - hor_int->y)
			/ tan(angle));
}

t_wall	find_hor_intercept(t_data *data, double angle)
{
	t_pos	hor_int;
	t_wall	hor_wall;

	find_first_hor(data, angle, &hor_int, &hor_wall);
	while (!is_off_map(data, hor_int) && !is_wall(data, hor_int))
	{
		if (angle < M_PI)
		{
			hor_int.y -= BLOCK_RES;
			hor_int.x += BLOCK_RES / tan(angle);
		}
		if (angle >= M_PI)
		{
			hor_int.y += BLOCK_RES;
			hor_int.x -= BLOCK_RES / tan(angle);
		}
	}
	if (is_off_map(data, hor_int))
		hor_wall.raylength = INFINITY;
	else
		hor_wall.raylength = sqrt(pow(fabs(data->player_pos.x - hor_int.x), 2)
				+ pow(fabs(data->player_pos.y - hor_int.y), 2));
	hor_wall.intercept.x = hor_int.x;
	hor_wall.intercept.y = hor_int.y;
	return (hor_wall);
}

void	find_first_vert(t_data *data, double angle, t_pos *vert_int,
		t_wall *vert_wall)
{
	if (angle > M_PI_2 && angle < 3 * M_PI_2)
	{
		vert_int->x = ((int)(data->player_pos.x / BLOCK_RES) * BLOCK_RES);
		vert_wall->walldirection = 'E';
	}
	else
	{
		vert_int->x = ((int)(data->player_pos.x / BLOCK_RES) * BLOCK_RES)
			+ BLOCK_RES;
		vert_wall->walldirection = 'W';
	}
	vert_int->y = data->player_pos.y + ((data->player_pos.x - vert_int->x)
			* tan(angle));
}

t_wall	find_vert_intercept(t_data *data, const double angle)
{
	t_pos	vert_int;
	t_wall	vert_wall;

	find_first_vert(data, angle, &vert_int, &vert_wall);
	while (!is_off_map(data, vert_int) && !is_wall(data, vert_int))
	{
		if (angle > M_PI_2 && angle < 3 * M_PI_2)
		{
			vert_int.x -= BLOCK_RES;
			vert_int.y += BLOCK_RES * tan(angle);
		}
		if (angle <= M_PI_2 || angle >= 3 * M_PI_2)
		{
			vert_int.x += BLOCK_RES;
			vert_int.y -= BLOCK_RES * tan(angle);
		}
	}
	if (is_off_map(data, vert_int))
		vert_wall.raylength = INFINITY;
	else
		vert_wall.raylength = sqrt(pow(fabs(data->player_pos.x - vert_int.x), 2)
				+ pow(fabs(data->player_pos.y - vert_int.y), 2));
	vert_wall.intercept.x = vert_int.x;
	vert_wall.intercept.y = vert_int.y;
	return (vert_wall);
}
